...
| ------------------- | ------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------- |
| `task.create`       | `save_issue` (no `id`)                                       | `teamId`, `title`, `description?`, `stateId?`, `assignee?`, `labelIds?`, `dueDate?`, `estimate?`, `parentId?` | No `id` = create; embed `clientToken` in description as `[ct:<token>]`; sub-issues per Phase 5                                              |
| `task.move`         | `save_issue` (id + `stateId`)                                | `id`, `stateId` resolved by type                                                                              | Map `req.targetListOrSectionId`: `"open"` → `unstarted`, `"wip"` → `started`, `"done"` → `completed`; resolve via `config.states[]`         |
| `checklist.check`   | `save_issue` (sub-issue id + `stateId`)                      | `id: req.itemId`, `stateId`                                                                                   | `req.itemId` = sub-issue id; `complete` → type `completed`; `incomplete` → type `unstarted`; sub-issue created during `task.create` Phase 5 |
| `task.close`        | `save_issue` (id + `stateId`)                                | `id`, `stateId` resolved by type `completed`                                                                  | Same upsert tool as `task.move`; type-based → locale/rename-safe                                                                            |
...
| `task.sprint.set`   | `list_cycles` → `save_issue` (id + `cycleId`)                | Two-call; see § Sprint                                                                                        | Team-gated (`config.cycles.enabled`); `NOT_APPLICABLE` when disabled; see [`../SKILL.md`](../SKILL.md) § Sprint                             |
...
Idempotency: `save_issue { id: subIssueId, stateId }` is idempotent — re-applying the same state type is a no-op.
