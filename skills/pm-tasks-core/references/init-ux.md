...
Every `pm-tasks-<tool>` adapter exposes `npx @llodev/pm-tasks-<tool> init` and follows this four-step flow. Implementation library at `skills/pm-tasks-core/src/init-lib.ts` (compiled to `dist/init-lib.js`; consumers import via `@llodev/pm-tasks-core/init-lib`).
