...
description: >-
  ...
  task.due-date.set, task.assignee.add, task.comment.add, task.move,
  task.parent.set, task.estimate.set, task.sprint.set) consumed by adapters.
...
See [`references/crud-vocabulary.md`](references/crud-vocabulary.md). Ten verbs: `task.create`, `checklist.check`, `task.close`, `task.due-date.set`, `task.assignee.add`, `task.comment.add`, `task.move`, `task.parent.set` (adapter-optional), `task.estimate.set` (adapter-optional), `task.sprint.set` (adapter-optional; Linear first supporter). All idempotent (with `clientToken` rules for non-natural cases).
