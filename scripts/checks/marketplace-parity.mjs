...
// resolved from the entry's source.path (layout-agnostic), falling back to
// the flat skills/<name> layout for entries without one.
...
for (const plugin of marketplace.plugins) {
  const dir = plugin.source?.path ?? `skills/${plugin.name}`;
