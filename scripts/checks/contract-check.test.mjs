...
test("missing manifest → exit 1 with specific message", () => {
  const manifestPath = `${ROOT}/skills/pm-tasks-asana/manifest.json`;
  const backupPath = `${ROOT}/skills/pm-tasks-asana/manifest.json.bak`;
...
test("task.parent.set accepted as canonical verb — no namespace prefix required", () => {
  ...
  const skillMdPath = `${ROOT}/skills/pm-tasks-asana/SKILL.md`;
