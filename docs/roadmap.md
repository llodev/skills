...
Date: 2026-06-17. Baseline: `main` at commit `8046c39` (v1.5.0 merged via PR #8). Forward-looking roadmap — single source of truth for the project; each item carries **real status** (delivered / partial / pending) and **priority**.

**Active P0 plan:** _none._ The v1.3.2 → v1.4.0 → v1.5.0 cycle ([`plans/2026-06-16-pm-tasks-v1.3.2-v1.5-org-ts-migration.md`](plans/2026-06-16-pm-tasks-v1.3.2-v1.5-org-ts-migration.md)) delivered. Next P0 plan to be picked from §6 below.
...
Last 3 releases. Older history lives in each package's `CHANGELOG.md` (auto-generated by Changesets) and in the git log — no point shadow-tracking it here.

| Release        | Theme                                                                                  | Outcome                                                                                                                                               |
| -------------- | -------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------- |
| v1.5.0 (PR #8) | Adapters TS + `@llodev/pm-tasks-testkit` + 7th canonical verb `task.move` + E2E canary | `@llodev/pm-tasks-core@1.5.0` (`task.move` on public API) · `@llodev/pm-tasks-{asana,trello}@1.4.0` (TS migration) · `@llodev/pm-tasks-testkit@0.1.0` |
| v1.4.0 (PR #7) | `pm-tasks-core` TypeScript migration                                                   | `@llodev/pm-tasks-core@1.4.0` — `src/init-lib.ts` + `src/i18n/registry.ts` + Vitest 2.x + `dist/` exports with `.d.ts` types                          |
| v1.3.2 (PR #6) | Repo organization                                                                      | `scripts/` reorganized into `checks/snapshots/shell` · SKILL.md translations dropped · localized READMEs moved to `docs/i18n/`                        |
